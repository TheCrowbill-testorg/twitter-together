...
const fs = require('fs');
const delay = 180 * 60 * 1000; // 180 minutes in milliseconds
let lastTweetTimestamp;

async function readLastTweetTimestamp() {
  try {
    const data = await fs.promises.readFile('lasttweettimestamp.txt', 'utf8');
    lastTweetTimestamp = data;
  } catch (error) {
    console.error(error);
  }
}  
...
async function cooldownTimer() {
  const now = Date.now();
  const cooldown = lastTweetTimestamp + delay;
  if (cooldown > now) {
    const remaining = cooldown - now;
    console.log(`Cooldown timer: ${remaining}ms remaining`);
...
async function tweet({ twitterCredentials }, tweetData, tweetFile) {
  console.log(`${timestamp()} Sending tweet...`);
  ...
  const tweetResult = await handleTweet(client, self.data, tweetData, tweetFile);
  
  console.log(`${timestamp()} Tweet sent successfully`);

  // Update the timestamp of the last successful tweet
  lastTweetTimestamp = timestamp()

  async function saveLastTweetTimestamp() {
   try {
      await fs.promises.writeFile('lasttweettimestamp.txt', lastTweetTimestamp);
    } catch (error) {
    console.error(error);
    }
  }
